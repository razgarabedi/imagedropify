...
const POST_UPLOAD_DELAY_MS = 1000; // 1 second delay
...
export async function uploadImage(
  ...
  try {
    ...
    try {
      ...
      // ---- WORKAROUND DELAY ----
      // Introduce a small delay to allow filesystem/permissions to propagate
      // before Next.js/Nginx attempts to access the file for optimization/display.
      // Adjust POST_UPLOAD_DELAY_MS (default 1000ms) as needed.
      // This is a workaround for server-side file visibility/permission timing issues.
      if (POST_UPLOAD_DELAY_MS > 0) {
        console.log(`uploadImage: Waiting ${POST_UPLOAD_DELAY_MS}ms after file write for ${filePath}`);
        await new Promise(resolve => setTimeout(resolve, POST_UPLOAD_DELAY_MS));
      }
      // ---- END WORKAROUND DELAY ----
      ...
      // Revalidate the specific shared folder page if it exists (more targeted)
      // This would require knowing active share IDs for this folder, complex to do here.
      // A broader revalidation of /share/* might be too aggressive or not work as expected.
      // For now, revalidating common paths is the approach.
...
export async function getUserImages(userIdFromSession?: string, limit?: number, targetFolderName?: string | null): Promise<UserImage[]> {
  ...
  if (targetFolderName === null) { 
    ...
    if (!foldersToScan.includes(DEFAULT_FOLDER_NAME)) { 
  ...
  for (const folderName of foldersToScan) {
    ...
    try {
      ...
      const resolvedUploadDirBase = path.resolve(path.join(UPLOAD_DIR_BASE_PUBLIC, userId)); 
      ...
      if (!resolvedUserFolderBaseDir.startsWith(resolvedUploadDirBase + path.sep) &&
          ...
          resolvedUserFolderBaseDir === path.resolve(UPLOAD_DIR_BASE_PUBLIC) 
          ) {
...
export async function calculateUserTotalStorage(userId: string): Promise<number> {
  ...
  const allUserImages = await getUserImages(userId, undefined, null); 
...
export async function createFolderAction(
    ...
    if (!userId) {
        ...
        if (!tokenExists) {
            ...
            // Token exists but getCurrentUserIdFromSession returned null, implying token is invalid/expired
